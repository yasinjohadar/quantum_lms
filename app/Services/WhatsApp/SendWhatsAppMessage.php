...
class SendWhatsAppMessage
{
    ...
    /**
     * Send template message immediately (without queue).
     */
    public function sendTemplateNow(
        ...
        try {
            ...
            $response = $providerInstance->sendTemplate($to, $templateName, $language, $components);
            ...
            Log::channel('whatsapp')->info('WhatsApp template message sent immediately', [
                'message_id' => $message->id,
                'meta_message_id' => $response->metaMessageId,
                'to' => $to,
            ]);
            ...
            Log::channel('whatsapp')->error('Failed to send immediate WhatsApp template message', [
                'message_id' => $message->id,
                'error' => $e->getMessage(),
                'details' => $e->getDetails(),
            ]);
            ...
            Log::channel('whatsapp')->error('Exception sending immediate WhatsApp template message', [
                'message_id' => $message->id,
                'error' => $e->getMessage(),
            ]);
