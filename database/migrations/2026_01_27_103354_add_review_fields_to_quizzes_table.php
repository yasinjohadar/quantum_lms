...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::table('quizzes', function (Blueprint $table) {
            ...
            $table->timestamp('submitted_for_review_at')->nullable()->after('reviewed_at')->comment('تاريخ إرسال الاختبار للمراجعة');
            
            $table->foreign('reviewed_by')->references('id')->on('users')->onDelete('set null');
        });

        // تحديث الاختبارات الموجودة
        DB::table('quizzes')->where('is_published', true)->update(['review_status' => 'approved']);
        DB::table('quizzes')->where('is_published', false)->update(['review_status' => 'draft']);
    }
