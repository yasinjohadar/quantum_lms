...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::table('assignments', function (Blueprint $table) {
            ...
            $table->timestamp('submitted_for_review_at')->nullable()->after('reviewed_at')->comment('تاريخ إرسال الواجب للمراجعة');
            
            $table->foreign('reviewed_by')->references('id')->on('users')->onDelete('set null');
        });

        // تحديث الواجبات الموجودة
        DB::table('assignments')->where('is_published', true)->update(['review_status' => 'approved']);
        DB::table('assignments')->where('is_published', false)->update(['review_status' => 'draft']);
    }
