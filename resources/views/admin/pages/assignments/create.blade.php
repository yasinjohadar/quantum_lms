    ...
    <div class="main-content app-content">
        <div class="container-fluid">
            <div class="page-header d-flex justify-content-between align-items-center my-4">
                <h5 class="page-title mb-0">إضافة واجب جديد</h5>
                    ...
                    <form method="POST" action="{{ route('admin.assignments.store') }}">
                        ...
                        <div class="row g-3">
                            ...
                            <div class="col-md-12">
                                ...
                                <textarea name="instructions" class="form-control @error('instructions') is-invalid @enderror" rows="5">{{ old('instructions') }}</textarea>
                                @error('instructions')
                                <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="col-md-6">
                                ...
                                <select name="assignable_type" id="assignable_type" class="form-select @error('assignable_type') is-invalid @enderror" required>
                                    ...
                                    <option value="App\Models\Subject" {{ old('assignable_type', $assignableType == 'subject' ? 'App\Models\Subject' : '') == 'App\Models\Subject' ? 'selected' : '' }}>مادة</option>
                                    <option value="App\Models\Unit" {{ old('assignable_type', $assignableType == 'unit' ? 'App\Models\Unit' : '') == 'App\Models\Unit' ? 'selected' : '' }}>وحدة</option>
                                    <option value="App\Models\Lesson" {{ old('assignable_type', $assignableType == 'lesson' ? 'App\Models\Lesson' : '') == 'App\Models\Lesson' ? 'selected' : '' }}>درس</option>
                                </select>
                            ...
                            <div class="col-md-6" id="late_penalty_div" style="display: none;">
                                ...
                                <input type="number" name="late_penalty_percentage" class="form-control @error('late_penalty_percentage') is-invalid @enderror"
                                       value="{{ old('late_penalty_percentage', 0) }}" min="0" max="100" step="0.01">
                            ...
                            <div class="col-md-4">
                                ...
                                <input type="number" name="max_file_size" class="form-control @error('max_file_size') is-invalid @enderror"
                                       value="{{ old('max_file_size', 10) }}" min="1" max="100" required>
                                @error('max_file_size')
                                <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="col-md-4">
                                ...
                                <input type="number" name="max_files_per_submission" class="form-control @error('max_files_per_submission') is-invalid @enderror"
                                       value="{{ old('max_files_per_submission', 5) }}" min="1" max="20" required>
                            ...
                            <div class="col-md-4">
                                ...
                                <div class="d-flex flex-wrap gap-2 mt-2">
                                    @php
                                        ...
                                        $oldTypes = old('allowed_file_types', []);
                                    @endphp
                            ...
                            <div class="col-md-12">
                                ...
                                <select name="grading_type" class="form-select @error('grading_type') is-invalid @enderror" required>
                                    <option value="manual" {{ old('grading_type', 'manual') == 'manual' ? 'selected' : '' }}>تصحيح يدوي</option>
                                    <option value="auto" {{ old('grading_type') == 'auto' ? 'selected' : '' }}>تصحيح تلقائي</option>
                                    <option value="mixed" {{ old('grading_type') == 'mixed' ? 'selected' : '' }}>مزيج</option>
                                </select>
                                @error('grading_type')
                                <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        console.log('Page loaded, initializing assignment form...');
