<div class="modal fade" id="deleteLibraryCategory{{ $category->id }}" tabindex="-1" aria-hidden="true">
        ...
        <div class="modal-content border-0 rounded-4">
            <div class="border-0 text-center pt-4 px-4">
                <div class="d-inline-flex align-items-center justify-content-center mb-3">
                    ...
                    <h5 class="modal-title mb-0 fw-bold">تأكيد حذف التصنيف</h5>
                </div>
            ...
            <div class="modal-body text-center pt-0 pb-3 px-4">
                <p class="mb-1 text-muted">هل أنت متأكد من حذف التصنيف التالي؟</p>
                <p class="fw-bold mb-1 text-danger" style="font-size:1.15rem;">{{ $category->name }}</p>
                <p class="mb-3 text-muted small">
                    لا يمكن حذف تصنيف يحتوي على عناصر مكتبة — احذف/انقل عناصره أولاً.
                </p>
            </div>

            <div class="modal-footer border-0 justify-content-center pb-4">
                <button type="button" class="btn btn-outline-secondary px-4 me-2" data-bs-dismiss="modal">إلغاء</button>
                <form action="{{ route('admin.library.categories.destroy', $category->id) }}" method="POST" class="d-inline">
                    @csrf
                    @method('DELETE')
