...
<div class="main-content app-content gami-page">
    <div class="container-fluid">
        ...
        @include('admin.pages.gamification.partials.hero', [
            'gamiTitle' => 'المهام الأسبوعية',
            'gamiIcon' => 'bi-calendar-week',
            'gamiBreadcrumbs' => [
                ['label' => 'الرئيسية', 'url' => route('admin.dashboard')],
                ['label' => 'نظام التحفيز', 'url' => route('admin.gamification.index')],
                ['label' => 'المهام الأسبوعية', 'active' => true],
            ],
            'gamiStatValue' => $tasks->count(),
            'gamiStatLabel' => 'مهمة',
            'gamiHeroActions' => '<a href="' . route('admin.weekly-tasks.create') . '" class="btn btn-sm btn-primary"><i class="bi bi-plus-circle me-1"></i> إضافة مهمة أسبوعية جديدة</a>',
        ])

        @include('partials.gamification-help-box', ['helpKey' => 'admin.weekly_tasks', 'showQueueStatus' => true])
        ...
        <div class="gami-card gami-card--flush">
            <div class="gami-card__header">
                <div class="d-flex align-items-center gap-2">
                    ...
                    <span>قائمة المهام الأسبوعية</span>
                </div>
            </div>
                ...
                <div class="gami-table-wrap">
                    <table class="table gami-table align-middle mb-0">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>
                                    <div class="gami-item-cell">
                                        <span class="gami-item-icon"><i class="bi bi-calendar-week"></i></span>
                                ...
                                <td>{{ $task->start_day_name }} - {{ $task->end_day_name }}</td>
                                <td>
                                    @if($task->is_active)
                                ...
                                <td>
                                    <div class="row-action-bar">
                                        <a href="{{ route('admin.weekly-tasks.edit', $task) }}" class="row-action-btn row-action-btn--primary" title="تعديل">
                                            <i class="bi bi-pencil"></i>
                                        </a>
                                        <form action="{{ route('admin.weekly-tasks.destroy', $task) }}" method="POST" class="row-action-form" onsubmit="return confirm('هل أنت متأكد من الحذف؟')">
                                            @csrf
                                            @method('DELETE')
                            ...
                            <tr>
                                <td colspan="7">
                                    <div class="gami-empty">لا توجد مهام أسبوعية</div>
                                </td>
                            </tr>
                            @endforelse
                        </tbody>
